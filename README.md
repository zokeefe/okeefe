...
- **people**: An array of person records with structured naming and location references. To minimize file clutter, optional fields with null values may be omitted entirely:
  ...
  - **Optional Fields**: All other properties (`middle_names`, `nicknames`, and `nationalities` as lists of strings, `maiden_name`, `alt_lang_name`, `mother`, `father`, and `meta` attributes such as `birth_location_id`, `residence_location_id`, `birth_year`, and `birthday`) are optional and should be excluded when unrecorded. Explicit `nationalities` arrays override automatically deduced historical citizenship.
